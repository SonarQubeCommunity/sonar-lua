...
/*table constructor*/
    ...
    builder.withSquidAstVisitor(new SourceCodeBuilderVisitor<LexerlessGrammar>(new SourceCodeBuilderCallback() {
      ...
      public SourceCode createSourceCode(SourceCode parentSourceCode, AstNode astNode) {
        seq++;
       // String functionName = astNode.getFirstChild(LuaGrammar.FUNCt).getFirstChild(LuaGrammar.BLOCK);
        //SourceFunction function = new SourceFunction("function"  + astNode.getToken().getLine());
        SourceFunction function = new SourceFunction("function" + seq);
        function.setStartAtLine(astNode.getTokenLine());
        return function;
      }
    }, LuaGrammar.FUNCTION,LuaGrammar.FUNCSTAT,LuaGrammar.FUNCTIONCALL));

    builder.withSquidAstVisitor(CounterVisitor.<LexerlessGrammar>builder()
      .setMetricDef(LuaMetric.FUNCTIONS)
      .subscribeTo(LuaGrammar.FUNCTION,LuaGrammar.FUNCSTAT,LuaGrammar.FUNCTIONCALL)
      .build());
